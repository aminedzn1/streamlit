...
filt = st.multiselect("Filter columns", options = data_budg.columns, default = data_budg.columns )

st.table(data_budg[filt])

data_func = pd.read_csv(r'data/sms_budget_f.csv')

filt2 = st.multiselect("Filter columns", options = data_func.columns, default = data_func.columns )
